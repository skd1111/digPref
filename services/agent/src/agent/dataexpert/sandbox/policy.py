        ...
        # 2. 禁止的内置函数调用
            ...
            if isinstance(node.func, ast.Name):
                if node.func.id in BLOCKED_CALLS:
                    raise SandboxViolationError(f"禁止调用内置函数 '{node.func.id}'", node)
            elif isinstance(node.func, ast.Attribute):
                # 检查 os.system / subprocess.run 等
                attr_name = node.func.attr
                if attr_name in ("system", "popen", "exec", "execvp", "spawn"):
                    raise SandboxViolationError(f"禁止调用危险方法 '.{attr_name}()'", node)

        # 3. 禁止的属性访问
        elif isinstance(node, ast.Attribute):
            if node.attr in BLOCKED_ATTRS:
                raise SandboxViolationError(f"禁止访问属性 '{node.attr}'（防沙箱逃逸）", node)
...
def _check_import(module_name: str, node: ast.AST) -> None:
    ...
    if top_level in BLOCKED_IMPORTS or module_name in BLOCKED_IMPORTS:
        raise SandboxViolationError(f"禁止导入模块 '{module_name}'（安全黑名单）", node)

    # 白名单校验
    if top_level not in ALLOWED_IMPORTS and module_name not in ALLOWED_IMPORTS:
        raise SandboxViolationError(
            f"模块 '{module_name}' 不在白名单内（允许：{', '.join(sorted(ALLOWED_IMPORTS))}）",
            node,
        )
