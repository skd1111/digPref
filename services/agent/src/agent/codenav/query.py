...
class SymbolQuery:
    ...
    def search(self, name: str, kind: Optional[str] = None, limit: int = 10) -> list[Symbol]:
        ...
        if not name:
            ...
            return [
                Symbol(
                    ...
                    signature=r[5], parent_class=r[6], language=r[7],
                )
                for r in rows
            ]
    ...
    def get_status(self) -> IndexStatus:
        """返回索引状态统计。"""
        with self._connect() as conn:
            total_files = conn.execute("SELECT COUNT(DISTINCT file_path) FROM symbols").fetchone()[0]
