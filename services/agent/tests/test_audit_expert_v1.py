...
class TestComplianceDual:
    ...
    def test_destructive_op_for_high_risk(self):
        ...
        checks = run_compliance_checks(
            task_id="t", risk_level=RiskLevel.HIGH,
...
class TestApiV1:
    ...
    def client(self, tmp_path, monkeypatch):
        ...
        from agent.audit_expert.store import (
            reset_default_storage, get_default_storage,
        )
    ...
    def test_dual_first_approve(self, client):
        ...
        resp = client.post(f"/audit/tasks/{task_id}/dual-first", json={
            "actor": "alice",
    ...
    def test_dual_first_requires_dual(self, client):
        ...
        resp = client.post(f"/audit/tasks/{task_id}/dual-first", json={
            ...
            "mfa_verified": True, "totp_code": None, "use_rsa": False,
        })
        assert resp.status_code == 400
    ...
    def test_dual_second_requires_dual_first(self, client):
        ...
        task_id = create["task_id"]
        resp = client.post(f"/audit/tasks/{task_id}/dual-second", json={
            "actor": "bob", "reason": "second",
            "mfa_verified": True, "totp_code": None, "use_rsa": False,
        })
        assert resp.status_code == 400
    ...
    def test_dual_second_different_actor(self, client):
        ...
        task_id = create["task_id"]
        totp = get_current_totp_for_user("alice")
        client.post(f"/audit/tasks/{task_id}/dual-first", json={
            "actor": "alice", "reason": "first",
            "mfa_verified": True, "totp_code": totp, "use_rsa": True,
        })
        # 同 actor 调 dual-second
        resp = client.post(f"/audit/tasks/{task_id}/dual-second", json={
            "actor": "alice", "reason": "second",
            "mfa_verified": True, "totp_code": totp, "use_rsa": True,
        })
        assert resp.status_code == 400
    ...
    def test_dual_full_flow(self, client):
        ...
        totp = get_current_totp_for_user("alice")
        resp1 = client.post(f"/audit/tasks/{task_id}/dual-first", json={
            "actor": "alice", "reason": "first approval",
            "mfa_verified": True, "totp_code": totp, "use_rsa": True,
        })
        assert resp1.status_code == 200, f"first failed: {resp1.json()}"
        resp2 = client.post(f"/audit/tasks/{task_id}/dual-second", json={
            "actor": "bob", "reason": "second approval",
            "mfa_verified": True, "totp_code": totp, "use_rsa": True,
        })
    ...
    def test_verify_chain_after_dual(self, client):
        ...
        client.post(f"/audit/tasks/{task_id}/dual-second", json={
            "actor": "bob", "reason": "second",
            "mfa_verified": True, "totp_code": totp, "use_rsa": True,
        })
