...
def test_tree_guard_accepts_valid_depth():
    """depth=1 + total=0 → 通过"""
    spec = SubAgentSpec(
        sub_agent_id='s1', parent_run_id='p', depth=1,
        task_type='plan', task_description='test',
    )
...
def test_tree_guard_accepts_max_depth():
    ...
    spec = SubAgentSpec(
        sub_agent_id='s2', parent_run_id='p', depth=MAX_DEPTH,
        task_type='plan', task_description='test',
    )
    enforce_tree_limits(spec, current_total_nodes=0)
...
def test_tree_guard_rejects_depth_too_large():
    """depth > MAX_DEPTH → 抛"""
    spec = SubAgentSpec(
        sub_agent_id='s3', parent_run_id='p', depth=MAX_DEPTH + 1,
        task_type='plan', task_description='test',
    )
...
def test_tree_guard_rejects_total_too_large():
    ...
    spec = SubAgentSpec(
        sub_agent_id='s4', parent_run_id='p', depth=1,
        task_type='plan', task_description='test',
    )
    with pytest.raises(TreeLimitExceeded) as exc:
...
def test_tree_guard_rejects_depth_zero():
    """depth<1 → 抛（最低 1）"""
    spec = SubAgentSpec(
        sub_agent_id='s5', parent_run_id='p', depth=0,
        task_type='plan', task_description='test',
    )
    with pytest.raises(TreeLimitExceeded) as exc:
        enforce_tree_limits(spec, current_total_nodes=0)
...
def test_report_validate_semantic_ok():
    ...
    r = SubAgentReport(
        sub_agent_id='r1', parent_run_id='p',
...
def test_report_validate_semantic_ok_requires_summary():
    ...
    r = SubAgentReport(
        sub_agent_id='r2', parent_run_id='p',
        status=SubAgentStatus.OK,
        started_at='2026-07-22T00:00:00Z',
        summary='   ',
    )
...
def test_report_validate_semantic_err_requires_error():
    ...
    r = SubAgentReport(
        sub_agent_id='r3', parent_run_id='p',
...
def test_report_validate_semantic_confidence_bounds():
    """confidence > 1 → 错误"""
    r = SubAgentReport(
        sub_agent_id='r4', parent_run_id='p',
        status=SubAgentStatus.OK,
        started_at='2026-07-22T00:00:00Z',
        summary='ok',
        confidence=1.5,
...
def test_spec_minimal():
    """只填必填字段也能构造成功"""
    s = SubAgentSpec(
        sub_agent_id='s', parent_run_id='p',
