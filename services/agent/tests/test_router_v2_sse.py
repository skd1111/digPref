...
def test_engine_route_request_emits_llm_route_decided():
    ...
    backends = [
        LLMBackend(
            name="local-1", type="local", base_url="http://localhost",
            model_name="test", data_residency="local", enabled=True, role="execution",
        ),
    ]
    metrics = MetricsRecorder()
    eng = RouterEngine(
        backends=backends, budget=BudgetController(),
        breakers=CircuitBreakerRegistry(), metrics=metrics,
    )

    decision = eng.route_request(
        task_kind="intent", category=TaskCategory.SIMPLE,
        sensitivity=Sensitivity.PUBLIC, request_id="test-req-1",
    )
...
def test_fallback_emits_llm_degraded_when_all_fail():
    ...
    from agent.llm.fallback import with_fallback, LLMBackendError
...
def test_engine_no_candidates_still_emits():
    ...
    backends = [
        ...
        LLMBackend(
            name="cloud-1", type="cloud", base_url="https://api.openai.com/v1",
            model_name="gpt-4o", data_residency="cloud",
            enabled=True, role="execution",
        ),
    ]
    eng = RouterEngine(
        backends=backends, budget=BudgetController(),
        breakers=CircuitBreakerRegistry(), metrics=MetricsRecorder(),
    )
    decision = eng.route_request(
        task_kind="plan", category=TaskCategory.COMPLEX,
