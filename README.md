...
| 专家团资产                  | 团+成员两级结构化/ 设置页独立维护（CRUD + 导入导出）/ 运营工作台点业务自动选择（Skill 预设 → LLM 本地→内网→云端降级 → 关键词）+ 输入栏手选 + 会话自动注入；Skill 预设默认专家团/材料/交付物三字段；资产包格式 zip = 根目录`team.yaml`（提示词/成员定义）+ `templates/`（交付物文档模板 docx/md），导入一次到位、导出整团打包；种子：`docs/expert-team-seeds/due-diligence-team.zip`                                          |
