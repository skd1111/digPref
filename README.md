...
| 双框架智能体（Phase 18）    | ModeRouter 智能路由 Coding / Work 双模式 + 任务分解 + Auto-Repair 自动修复 + 工具循环预算 24 轮 + 停滞熔断（连续 3 轮零成功才终止，有进展的长链任务不受影响）+ 任务进度待办列表（update_todos 伪工具驱动，进度横幅悬浮在会话页签下方常驻：不占对话区版面，滚动/翻历史不丢失，全部完成后自动收起，BUGFIX #150）；选项式追问支持粗体选项（**A. xxx**）成卡、手打孤立编号回复自动展开为结构化确认文本防丢上下文（BUGFIX #149）；执行块人性化文案：工具名显示为中文动作短语（写文件/执行命令/创建 Office 文档…）+ 进行中/已完成/失败状态词，点击动作跳转思维链对应步骤并闪烁（BUGFIX #153）；SSE 终止信号三层防丢（done 移出 finally 安全路径 / 无图块熔断 / Rust 桥 stream_closed 兜底，BUGFIX #152）                                                                                                                                                                                                                       |
| 内置工具层                  | 45 个进程内工具（文件读写/搜索/计算器/正则/时间日期含农历/相对日期解析/shell 等）+ 文档处理工具族（file_to_markdown 文件转 Markdown / Excel 查询导出 / PDF 合并拆分 / Word 生成）+ V9 Office 工具族（OfficeCLI 引擎：office_read / office_edit / office_create / office_validate，写操作走 HITL）+ klogg 式大文件只读查看与搜索（突破 100MB 限制）+ 路径沙箱 + HITL 前置闸门                                                                                                                                                                     |
...
| Skill / MCP 生态            | Skill YAML + MCP JSON Schema / 关键词路由（归一化 + 中英混排核心 token 兜底，「做一个…的ppt」也能命中 office_pptx_designer）/ LLM 意图分类端侧探活门 / 热加载 / 多项目隔离；追问/修改轮 skill 粘性继承（上轮命中的技能随对话透传，防「太丑了重做」裸生成）；首次启动自动播种内置种子（Office 生成规范四件套：office_doc_writer / office_excel_analyst / office_pptx_designer / visual_deck_designer，蒸馏自 MIT/Apache-2.0 开源 skill 与开源设计体系美学思想自研，用户可修改覆盖、删除不复活）                                                                                                                                                                                                                                                                          |
| 任务级工作空间与产物治理     | 统一工作空间（默认安装目录/workspace，设置页可自定义）+ 任务级文件夹（一个聊天页签 = 一个任务文件夹，运行期产生的所有文件都落在里面）；创建类工具（含 office_create）输出路径调度入口统一改写，绝对路径豁免收紧为「必须出现在用户对话原文」防模型自造路径散落用户目录；交付后弹验收清理卡〔清理中间文件（保留产物）/全部保留〕，只删任务目录内非产物文件；对话中的文件路径可点击直接用默认程序打开、右键在资源管理器定位/复制/编辑器打开                                                                                                                                                                                                                                                |
