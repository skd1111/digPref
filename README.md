...
| 存储   | SQLite（audit / sessions / router 等多库物理隔离）+ sqlite-vec 向量存储 + OS Keychain 凭证 |
...
| 模块                        | 说明                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 基础架构                    | Tauri 桌面壳 / 四象限 IDE 布局 / LangGraph 状态机 / HITL 审批闸门 / SSE 事件桥 / SQLite 审计 / Agent 启动闸门（Agent(:8765) 未就绪期间整屏模糊 + 禁点禁快捷键，/health 一通自动放行；首轮超时给重试 / 重启 Agent / 查看 eaide.log / 跳过闸门四个出口，且后台每 3s 复探自动放行，BUGFIX #191）                                                                                                                                                                                                                                                                                                                                     |
...
| 结构化意图识别              | Intent Router 一次分析产出改写句/细分意图/实体/追问信号/风险等级；三级快速路径：关键词 → 向量语义路由（semantic-router 模式，本地 embedding 零 LLM，示例句向量缓存持久化在 sqlite-vec）→ LLM 分析，明确信号直达路由省掉决策器调用；动态 Few-Shot 案例库向量同落 sqlite-vec（intent_memory.db）                                                                                                                                                                                                                                 |
| 向量存储基建（sqlite-vec） | 全仓向量统一落 SQLite sqlite-vec 扩展（vec0 虚拟表，`agent/vector_store.py` 单一入口），与进程内 ONNX 向量模型（bge-small-zh-v1.5）端侧闭环；六处消费点：意图 Few-Shot 案例 / 语义路由示例句 / L2 语义缓存（router.db 持久层，重启后仍可语义命中）/ 财税规则库与 NL2SQL 表向量持久缓存（重启免全量重新向量化）/ 知识库分块检索；扩展不可用全部静默回退（返空/现算），不阻塞主链路                                                                                                                                                                                                                                 |
...
| 回答逐字流式输出            | 终答从「节点跑完整段下发」升级为逐 token 流式（`answer_delta` SSE 事件，三处同步）：responder 全部 summarise 路径（MAIN_AGENT 直答 / 中段闲聊 / 工具结果综合 / 中文重写 / 子 Agent 汇总）接入 Ollama NDJSON / OpenAI SSE 流式原语；`ThinkBlockFilter` 增量抑制思考块防漏；草稿气泡与终稿同 msgId 原地覆盖收敛（#142 机制）；首 delta 前失败切下一后端、已发 delta 后失败回退非流式，行为不劣化；`answer_stream_enabled` 一键回退整段下发                                                                                                                                                                                                                                                                          |
...
| 专家验收工作流（运营模式）  | 取代传统大 Chat：按专家上传材料（拖拽 / Ctrl+V 粘贴入件）→ AI 审核验收（结论+关键要素+证据链，幻觉证据丢弃，低置信标红）→ 专家迷你提问（人设 + 上传知识库 RAG 制度出处引用）→ 交付标准逐项确认 → 交付物 zip 打包导出（交付文件/检查结果/问答记录/业务小结/报告初稿 docx 模板占位符渲染）+ 多文档交叉比对防呆 + 人工改判纠错样本闭环（全操作审计）                                                                            |
...
| 文档风险审核                | PDF/DOCX/TXT/MD 解析 → LLM 分类（合同/制度/公告/标书 × 合规/法律/数据安全/资金）→ 分块风险分析 → 风险位置正文高亮 + 知识库/案例库引用依据（审核专家模式文档审核 Tab）；用户上传参考资料经本地知识库混合检索（FTS5 BM25 + sqlite-vec 向量 + RRF + ONNX reranker，父子分块回喂）注入 `{{rag_context}}` 槽位，带编号引用与禁止编造指令                                                                                                                                                                                                                                                        |
...
| 模块                     | 现状                                                                                                         | 未实现部分                                                      |
| ------------------------ | ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------- |
| 本地小模型 + 知识库      | **本地混合检索 RAG 已落地**（审核专家 + 聊天共用）：SQLite FTS5 原生 BM25（jieba 分词）+ sqlite-vec 向量余弦 + RRF 融合 + ONNX cross-encoder reranker 重排；父子两层分块（small-to-big）+ 标题上下文前缀；文档级 CRUD + 上传入库（复制源文件/解析/分块/向量化/状态机）+ 前端知识库管理面板（审核页）+ 设置页 RAG 参数（`/settings/knowledge`，存 kb.db `kb_config` 表、查询期参数保存即热生效、索引期参数一键重建）；数据落 `data_root()/knowledge/`（kb.db 含参数表 + files/ 源文件 + 向量同库），复制即迁移、换机即用，模型漂移基于库内原文自愈重建；大模型知识库验证总开关（`rag_llm_enhance_enabled`，默认关 → 审核/聊天只走本地混合检索、零大模型调用）；知识库依据/已上传文件点击预览（md/txt/html/office 分流 + 应用内 PDF 内嵌）；embedding/FTS5/reranker 任一不可用均 best-effort 降级；**聊天召回真正接入作答（BUGFIX #192）**：`rag_retrieve` 召回经 `system_prompt_addon` → `summarise` 全链 `rag_context` 注入终答 prompt（此前只写 state、无读取方，模型看不到召回），文档/制度类查询且 BM25 命中相关资料时 `decompose` 走 RAG 优先据库直接作答（`rag_first_answer_enabled`），追问轮改用 `rewritten_query` 检索并拼上一轮主题；`semantic_route` 新增 `kb_lookup` 路由让「知识库里没有吗」类确认追问零 LLM 直出 `need_tool=False`；工具循环新增只读文件探测发散刹车（`tool_loop_fs_probe_soft/hard_limit`）+ 单 run 墙钟超时（`tool_loop_wall_clock_sec`），防跨盘 shell 扫描卡死 | LLM 增强检索阶段（HyDE / 查询扩展 / LLM 上下文前缀，受 `rag_llm_enhance_enabled` 总开关 + 各子开关控制，默认关；启用时走「模型管理」已启用模型）、聊天引用卡前端渲染（refs 已透出 trace）                        |
| 审核专家模式（金融审计） | V0 后端骨架（审批队列 + 签名链 + 合规规则）+ 文档审核子功能已交付（风险高亮 + 知识库引用）                   | 前端 AuditDashboard 完整工作台、Monaco Diff 审核、MFA、双人复核 |
| 本地图像处理             | V0 后端骨架（6 端点 + 任务表）+ OCR 真实后端（RapidOCR onnxruntime 端侧 PP-OCRv4，纯本地不出域；扫描件 PDF 经 pypdfium2 栅格化后识别，doc_review 仅无文本层时回退，chat 可经意图/工具触发）                                                                               | ONNX 超分 / 倾斜校正真实集成、前端 UI               |
...
│   │       ├── knowledge/     # 本地知识库混合检索（FTS5 BM25 + sqlite-vec 向量 + RRF + reranker，审核专家/聊天共用）
│   │       ├── vector_store.py # sqlite-vec 统一向量访问层（全仓向量存储单一入口）
...
│   │       ├── …              # codenav（索引/语法检查）/biznav/reqflow/skills/preview/doc_review（依据走上传 RAG）/audit/safety 等
...
>
> 📚 内置 `knowledge-base/` 已于 2026-09-04 **彻底移除**（不再打包、仓库目录删除）：文档审核「依据」与 ops 制度问答均已改走用户上传的本地 RAG 知识库（kb.db + files/，随数据根迁移、换机即用）；内置 grep 知识库、财税规则库与 audit_doc 演示脚本一并下线。
>
