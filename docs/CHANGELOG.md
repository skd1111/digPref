...
- **改动**：
  - **① 依据/列表可点预览**：`KbCitation`/协议 `DocKbRef`（[doc_review.py](../packages/shared-protocol/src/protocol/doc_review.py) + [doc-review.ts](../packages/shared-protocol/src/ts/doc-review.ts) 镜像）新增 `file_path`（命中知识文件绝对路径，展示标签仍不露文件名）；[knowledge.py](../services/agent/src/agent/doc_review/knowledge.py) 回填路径；`/knowledge/v1/docs` 的 `V1DocSummary` 新增 `file_path`（[api.py](../services/agent/src/agent/knowledge/api.py) `_resolve_stored_path` 由库内相对路径拼当前数据根，零绝对路径入库红线不变）。前端新增统一入口 `previewLocalFile`（[officePreviewStore.ts](../apps/desktop/src/store/officePreviewStore.ts)）按扩展名分流：docx/xlsx/pptx→OfficeCLI 渲染、md→Markdown 渲染、txt/csv→等宽纯文本、html→沙箱 iframe、pdf→WebView 内嵌、doc 等→系统默认程序；[FindingsPanel.tsx](../apps/desktop/src/components/doc-review/FindingsPanel.tsx) 依据来源变为可点击（stopPropagation 不误触详情弹窗），[KbPanel.tsx](../apps/desktop/src/components/doc-review/KbPanel.tsx) 文件名 + 「预览」按钮可点。
  - **② 大模型知识库验证总开关**：新增 `rag_llm_enhance_enabled`（默认关，硬闸）——关闭时 [hybrid_rag.py](../services/agent/src/agent/knowledge/hybrid_rag.py) 的 HyDE/查询扩展与 [ingestion.py](../services/agent/src/agent/knowledge/ingestion.py) 的上下文前缀即便子开关为真也**零大模型调用**，检索/入库只走本地 BM25+向量+RRF+reranker。
  - **③④ 参数落库 + 热生效 + 复制即用**：[rag_config.py](../services/agent/src/agent/knowledge/rag_config.py) 持久化从 `rag_config.json` 迁到 **kb.db 的 `kb_config` 表**（[storage.py](../services/agent/src/agent/knowledge/storage.py) 新增表 + `get_all_config`/`set_config_many`），与向量/源文件同一可复制单元；旧 JSON 一次性导入迁移；`save_rag_config` 改为**热应用**（查询期参数保存即生效、无需重启，`restart_required=False`），索引期参数（分块/父块/上下文前缀）回传 `needs_reindex` 供前端提示一键重建；[api.py](../services/agent/src/agent/knowledge/api.py) `v1_config_set` 保存后重置检索/入库单例使 top_k/reranker 按新参数重建，`v1_config_get` 增返 `index_time`。[KnowledgeRagPanel.tsx](../apps/desktop/src/views/settings/KnowledgeRagPanel.tsx) 加总开关 + 「改后需重建」标记 + 保存即热生效/重建提示 UX。
  - **⑤ 应用内 PDF 预览**：[tauri.conf.json](../apps/desktop/src-tauri/tauri.conf.json) 开启 `assetProtocol`（scope 限 app data/home/resource/temp）+ CSP `frame-src`；[Cargo.toml](../apps/desktop/src-tauri/Cargo.toml) 补 `protocol-asset` feature；`officePreviewStore.openPdf` 经 `convertFileSrc` 得 asset URL，[OfficePreviewPanel.tsx](../apps/desktop/src/components/office/OfficePreviewPanel.tsx) iframe 内嵌 + 「系统打开」兜底。
  - **⑥ 扫描件 PDF OCR**：[image_processing/ocr.py](../services/agent/src/agent/image_processing/ocr.py) 落地真实 `RapidOcrBackend`（rapidocr_onnxruntime 端侧 PP-OCRv4，纯本地不出域）+ 可复用同步入口 `recognize_image_sync`/`ocr_pdf_to_pages`（pypdfium2 栅格化），`get_default_backend` 可用时优先真实后端（chat 意图/工具即走此）；[doc_review/parser.py](../services/agent/src/agent/doc_review/parser.py) `_parse_pdf` **仅当无文本层（扫描件）** 才回退 OCR，受 `doc_review_pdf_ocr_enabled` 开关控制、依赖缺失静默退化。依赖新增 `pypdfium2`+`rapidocr-onnxruntime`（复用已有 onnxruntime），[eaide-agent.spec](../eaide-agent.spec) 收集 RapidOCR 模型（`collect_data_files`）+ pypdfium2 原生库（`collect_dynamic_libs`）。
  - **⑦ 审核依据全面改走上传 RAG + 下线内置知识库打包**：[doc_review/api.py](../services/agent/src/agent/doc_review/api.py) `_attach_kb_refs` 改为异步、按 finding 检索**用户上传的本地 RAG 库**（命中转 kb_refs，`file_path` 指回上传源文件→任务①的可点击预览因此指向用户文档），并移除基于内置财税规则库的 `{{rules}}` 注入与风险类型自动补全（依据统一走 `{{rag_context}}`）；**删除**内置 grep 模块 [doc_review/knowledge.py](../services/agent/src/agent/doc_review) 与财税规则库 `doc_review/fiscal_rules.py`（+ 对应测试 + `doc_review_kb_dir`/`doc_review_fiscal_*` 配置），[rules.py](../services/agent/src/agent/doc_review/rules.py) 仅留 `PolicyRule` + Noop 口子；`storage.resolve_file_path(doc_id)` 新增（doc_id→库内源文件绝对路径）；[eaide-agent.spec](../eaide-agent.spec) 移除 `('knowledge-base','knowledge-base')` 打包。ops 制度问答 / audit_doc 演示脚本（非审核模块）的处理见 ⑧。
  - **⑧ 内置 knowledge-base/ 彻底下线（ops 制度问答也改走 RAG + 删 audit-document demo + 物理删除目录）**：接用户「审核所有依据走 RAG、原先文档包括财税都删掉、不打包」的进一步决策，排查确认内置 `knowledge-base/` 仅剩两个非审核消费者——① [ops/api.py](../services/agent/src/agent/ops/api.py) 制度问答 `_search_local_knowledge`（**活功能**：运营工作台「专家验收工作流」迷你提问命中时附「📚 制度出处」）、② `audit_expert` 的 `/audit-document` 简化路径（**demo 死路径**：前端/Rust 均无入口，仅 CLI/curl 可达）。处理：**ops 制度问答改走上传 RAG**（`_search_local_knowledge` 改 async、`get_default_retriever().retrieve`，制度出处=上传文档标题、不再是内置文件名；删除 `_knowledge_base_dir`/`EAIDE_KB_DIR`/`_KB_STOPWORDS` 及 `import sys`）；**删除** `/audit-document` 端点 + `AuditDocumentRequest` + [scripts/audit_doc.py](../services/agent/scripts) + `test_audit_doc_ranking.py`；[test_ops_case.py](../services/agent/tests/test_ops_case.py) 制度问答用例改写为 monkeypatch RAG 检索器版；[eaide-agent.spec](../eaide-agent.spec) 注释、[README.md](../README.md)（专家验收/文档审核/目录树/📚 注记 4 处）、[onnx_embedding.py](../services/agent/src/agent/llm/onnx_embedding.py) 注释同步为「已彻底移除」。物理删除 `knowledge-base/`：84 文件中 **65 已删**，剩 19 个较大二进制（.doc/.docx/.pdf，如 237KB）+ 空目录因 DeleteFile 工具静默失效（报 success 实未删、文件经独占打开验证未锁）待手动 `Remove-Item -Recurse -Force knowledge-base` 清除。**验证**：改动模块 ruff 干净、ops/api.py 无新增 mypy 错（存量 bare-dict 债务 CI 不门禁 services 层）、**全量 pytest EXITCODE=0**（含改写的 ops 制度问答 RAG 版 + audit_expert v0/v1 + doc_review）。
- **影响**：知识库依据与已上传文件均可一键预览（含应用内 PDF）；**审核依据现全部来自用户上传的 RAG 知识库**（内置 grep 知识库/财税规则库下线、不再打包），未上传时依据为空；大模型验证默认关、审核只走本地混合检索；RAG 参数集中在设置页、存 kb.db、查询期改动即时生效（告别重启），拷 `knowledge/` 目录到新机即用；扫描件 PDF 现可 OCR 后参与审核/入库，chat 亦可经意图/工具触发 OCR；**ops 制度问答亦改走上传 RAG**（不再是内置 grep 关键词），audit-document demo 路径与 scripts/audit_doc.py 删除，内置 knowledge-base/ 彻底下线（不再打包、代码零引用；物理目录 65/84 已删、余 19 二进制+空目录待手动清）。安装包因 opencv 增约 60–90MB（另因去除内置 knowledge-base 略有抵消）。
- **验证**：后端 ruff/mypy --strict 干净、**全量 pytest EXITCODE=0**（新增 OCR 回退/RapidOCR 后端/kb.db 配置迁移+热应用/复制即用向量持久化/依据 file_path 等用例）；前端 tsc/eslint 零告警、**vitest 371 全绿**（新增预览分流 6 例 + kbPanel 预览 2 例 + findings 依据可点 2 例）；`cargo check` 通过（asset 协议/CSP 配置校验 + protocol-asset 编译）；真实 RapidOCR API 与引擎初始化实测通过（pypdfium2 5.12.1 + rapidocr 1.4.4）。

## 2026-09-03

### v2.132 — 知识库提升为审核专家顶层页签 + 修复打包后 sqlite-vec 向量检索全仓静默失效（BUGFIX #186）

- **背景**：① 用户要求把「知识库」从文档审核页内的子面板提升为与「审批工作台」「文档审核」平级的顶层页签。② 排查用户「上传后向量模型是否真运行/数据库是否真存/审核是否真用混合检索」时，从生产日志发现 `hybrid_search ... bm25=20 vec=0`（向量通道恒 0），直查生产 data_root 下所有 .db——**无任何 vec0 虚表**，定位为打包遗留重大缺陷。
- **改动**：① **顶层页签**：[AuditDashboard.tsx](../apps/desktop/src/views/AuditDashboard.tsx) `docTab` 布尔改三态 `tab: approval|doc|kb`，新增「知识库」TabButton；[KbPanel.tsx](../apps/desktop/src/components/doc-review/KbPanel.tsx) 由侧栏紧凑面板重写为全页视图（头部概览 + 上传条 + 全宽文件表格 + 重建索引/跳转检索参数）；[DocReviewDashboard.tsx](../apps/desktop/src/views/DocReviewDashboard.tsx) 移除左栏内嵌。② **BUGFIX #186**：[eaide-agent.spec](../eaide-agent.spec) 新增 `_sqlite_vec_binaries()`（`collect_dynamic_libs('sqlite_vec')` 把原生扩展 `vec0.dll` 放进 `_MEIPASS/sqlite_vec/`，与 `sqlite_vec.loadable_path()` 对齐）+ `hiddenimports` 补 `sqlite_vec`。此前该 .dll（运行时按路径 load_extension 加载、非 Python import 依赖）未被 PyInstaller 收集 → 打包 exe 里 `load_extension` 静默失败 → `CREATE VIRTUAL TABLE ... USING vec0` 建不了表 → 全仓向量检索（知识库/语义路由/L2 缓存/财税/NL2SQL）退化为纯关键词（开发/测试因 venv 里 .dll 在而全绿，极隐蔽，自 v2.123 即存在）。
- **影响**：重打包后向量通道恢复，混合检索才是真的 BM25+向量+RRF+rerank；存量知识库经「重建索引」基于库内原文重嵌入即可补齐向量（无需重传）。详见 [BUGFIX_LOG_2 #186](BUGFIX_LOG_2.md)。
- **验证**：日志核实上传入库与审核召回均已真实发生（`ingest done children=103`、`doc_review rag retrieve hits=5 chars=3422`、`onnx_embedding: loaded`、`reranker: loaded inputs=['input_ids','attention_mask']`）；前端 tsc/eslint 零告警，kbPanel/knowledgeRagPanel/docReviewDashboard/settingsRoutes 共 11 用例全绿；spec `py_compile` 通过，`collect_dynamic_libs('sqlite_vec')` 实测返回正确目标。

### v2.131 — 移除内置 PPT Master：ppt-master 技能包不再随包，PPT 技能改由用户自行上传（安装包 −~100MB）

- **背景**：以「安装包尽量小」为最高目标，决定不再内置 ppt-master（此前作为 Tauri `bundle.resources` 随包，含技能包 scripts/workflows/templates/references + deps 离线 wheel，约 100MB）。PPT 生成改为**用户按需自行上传技能**（经既有「设置 → 技能」/`POST /skills/import`，支持 zip 导入），内置种子 office_pptx_designer 一并移除。注：ppt-master 从不进 agent.exe（仅作 Tauri 资源），故本次精简的是**安装包**（约 −100MB），exe 本身几乎不变。
- **改动**：① **删除**：[ppt_master_bootstrap.py](../services/agent/src/agent/ppt_master_bootstrap.py)（解释器解析 + 离线装 deps + `{{PPT_MASTER_PYTHON}}` token 机制整套）、内置种子 `skills/seeds/office_pptx_designer.yaml`（+ fake_meipass 副本）、`test_ppt_master_bootstrap.py`、`infra/scripts/fetch-ppt-master.ps1`。② **回退注入**：[main.py](../services/agent/src/agent/main.py) 去掉启动期 `ensure_ppt_master_runtime()` 调用；[loop.py](../services/agent/src/agent/tools/loop.py) `_active_skill_addon` 与 [router.py](../services/agent/src/agent/skills/router.py) `build_system_prompt` 回退为直接拼 `skill.system_prompt`（移除 substitute_python_token）；router docstring 例子改泛化。③ **打包/构建**：[tauri.conf.json](../apps/desktop/src-tauri/tauri.conf.json) `bundle.resources` 移除 `vendor/ppt-master/`；[build-all.bat](../build-all.bat) 删除 ppt-master fetch + palette/type 图廊裁剪两步；release.yml 删除 Fetch PPT Master 步骤；ci.yml 两处占位去掉 vendor/ppt-master；spec、.gitignore 注释同步（`/vendor/ppt-master/` 保留 gitignore 仅防旧遗留误提交）。④ **测试**：conftest 去掉 `EAIDE_SKIP_PPT_BOOTSTRAP`；test_skills_seed / test_skills_api 的种子集合断言去掉 office_pptx_designer；[installerIncremental.test.ts](../apps/desktop/tests/installerIncremental.test.ts) 护栏翻转为「resources 不含 vendor/ppt-master 与 vendor/python」+ 泛化 BUGFIX #160 的 `**` 双星 glob 禁令。（其余测试把 office_pptx_designer 当内联 fixture id 借用、不加载真实种子，无需改。）
- **影响**：安装包再瘦 ~100MB（累计 v2.124/126/127/129/130/131 从 407MB 起已大幅缩减）。**行为变化**：全新安装不再自带 PPT 生成技能；需要 PPT 的用户自行经「设置 → 技能」导入 ppt-master 技能包（含其脚本/依赖，用户上传的技能自负依赖）。visual_deck_designer（HTML 演示，独立于 ppt-master）等其余内置种子不受影响。老安装若已播种 office_pptx_designer，其 data_root 副本不会被自动删除（seed 永不覆盖/删除既有），但已无对应 vendor/ppt-master 运行时，该技能会因找不到 SKILL_DIR 友好降级——建议老安装手动删除该技能。
- **验证**：agent 技能相关 39 用例全绿（skills_api/seed/router/loop_inject）+ `import agent.main` 无 ModuleNotFoundError；前端 installerIncremental + slashCommands 15 用例全绿；ruff check 通过；tauri.conf.json/release.yml/ci.yml 语法有效、spec `ast.parse` OK；生产代码（`**/src/**`）零 ppt-master 残留引用；mypy 仅 router.py 两处**既有历史负债**（104 行 union-attr、135 行缺注解，均在 HEAD 已存在、非本次触碰）。

### v2.130 — 打包前置修复 + prune 加固：.ps1 编码/无 pip venv 两个构建阻塞修复 + config/driver 精简改以 PyInstaller TOC 为准 + ppt deps cp314 落地

- **背景**：v2.129 后准备直接打包，前置核查暴露两个会让 build-all 失败/产出空依赖的真实 bug；且 v2.129 的 prune「venv 启发式」判定有误删隐患（构建 venv 若装 db-all extra，oracledb/pyodbc/clickhouse 会被误判为已冻结而删掉运行期唯一来源的 wheel）。本轮修 bug + 把 prune 判定改为以 PyInstaller 实际冻结结果为准 + 真正下载 cp314 wheel 落地。
- **改动**：① **.ps1 编码（#184）**：build-all.bat 用 Windows PowerShell 5.1 调用含中文的 .ps1，无 BOM 被按 GBK 误读→吞掉字符串终止符→整脚本解析失败（fetch-ppt-master FAIL 7、download_bge_reranker_onnx FAIL 3；pwsh7/CI 用 pwsh 不受影响，故此前被 ParseFile 掩盖）。给 4 个 .ps1（fetch-ppt-master / fetch-officecli / download_bge_reranker_onnx / download_bge_onnx）前置 UTF-8 BOM，5.1+pwsh7 均正确解析。② **fetch 取解释器（#185）**：`Resolve-Python` 返回裸 `py` 且不校验 pip，激活 uv `.venv`（无 pip）时 `py -m pip` → No module named pip，静默下 0 wheel（且已先清旧 cp312 → deps 变空）。改为 `Resolve-PipPython`：逐个探测 `-m pip --version` 退出码，优先 `py -3` 取真实解释器绕过 venv，全不可用则 throw。③ **prune TOC 加固**：[prune-driver-bundle.py](../infra/scripts/prune-driver-bundle.py) 判定口径从「分发名装在构建 venv ⇒ 会冻结」改为读 build/eaide-agent/ 的 PYZ-00.toc（纯 py 模块）+ PKG-00.toc（二进制/数据）解析 exe 真正冻结的顶层导入名集合（实测 346 个），对每个 wheel 解压取顶层导入名、**全部命中冻结集才判冗余删除**（保守：任一缺失即留）；TOC 不可得时回退旧 venv 比对并告警；新增 `--toc-dir`。彻底消除「venv 装了但 PyInstaller 未冻结其懒加载模块」导致的误删风险。④ **ppt deps cp314 落地**：重跑修好的 fetch，deps 换为 65 个 wheel（12 cp314 原生 + 4 abi3 + 余 none-any，52MB）+ requirements-offline.txt（18 核心包），无 cp312 残留。
- **验证**：4 个 .ps1 经 `powershell`(5.1) ParseFile 全 OK；fetch 实跑成功下 65 wheel；prune TOC 交叉校验实测——13 个应删包（numpy/pandas/cryptography/asyncpg/aiomysql/pymysql/openpyxl/certifi/urllib3/dateutil/six/tzdata/cffi）全部命中冻结集、6 个应留驱动（oracledb/pyodbc/clickhouse_connect/aioodbc/lz4/backports）全部未命中，--dry-run 保留 6 驱动、config/driver 维持 2.8MB；prune 脚本 ruff check/format + mypy --strict 零错误；bootstrap 15 用例全绿。已记 [BUGFIX_LOG_2](BUGFIX_LOG_2.md) #184/#185。
- **打包就绪**：deps 已 cp314、config/driver 已精简、build-all 调用的脚本编码/解释器解析均已修复——现在可直接 `build-all.bat` 打包。

### v2.129 — 安装包再瘦身：config/driver 构建期去冗余 + ppt deps 收窄到仅 3.14

- **背景**：以「安装包尽量小」为目标，在 v2.124/126/127 基础上再砍两块。核查发现 config/driver（本地实测 146MB）里绝大多数 wheel 与 exe 冻结副本重复（numpy/pandas/cryptography/asyncpg/aiomysql… 早已是 agent 依赖被 PyInstaller 冻结）；ppt deps 多版本 wheel 每版 ~24MB 且不可压缩。
- **改动**：① **config/driver 构建期精简**：新增 [prune-driver-bundle.py](../infra/scripts/prune-driver-bundle.py)——在构建 venv 内用 `importlib.metadata` 比对每个 wheel 的分发名，已安装（→ 会被冻结进 exe）的判为冗余删除，只保留 exe 未冻结的少数驱动（oracledb/clickhouse_connect/aioodbc/pyodbc/lz4/backports_zstd）；同时删除 `_site/`（driver_bootstrap 运行期从 wheel 幂等重解压，从不入包）。用分发名而非 top_level.txt（现代 wheel 常缺该文件、且有的把 docs/examples 也列进去，不可靠）。[build-all.bat](../build-all.bat) 新增 Step 1b 打包前调用（best-effort；config/driver 缺失/无 wheel 时 no-op；CI 用占位故不接 release.yml）。② **ppt deps 收窄到仅 3.14**：[fetch-ppt-master.ps1](../infra/scripts/fetch-ppt-master.ps1) `$Versions` 默认 `@("314")`（保留 `-Versions` 参数，需多版本一条命令拉齐）；[ppt_master_bootstrap.py](../services/agent/src/agent/ppt_master_bootstrap.py) `_SUPPORTED_MIN` 提到 `(3,14)`（min=max=3.14）；种子/spec/build-all/gitignore/release 文案同步 3.14。
- **实测收益**：config/driver 146MB → 2.8MB（本地实测释放 143MB：_site 115MB + 冗余 wheel ~28MB）；ppt deps 3 版本(~100MB) → 仅 3.14(~52MB)，省 ~48MB。config/driver 作为 tauri 资源单独落安装目录，不影响 agent.exe 本身。
- **权衡/注意**：① 保留的 6 个驱动运行期仍由 driver_bootstrap 从 wheel 解压提供，其余用 exe 冻结副本，功能不变；config/driver 未入 git、可重新放置，精简幂等。② ppt deps 仅带 3.14 wheel——**3.12/3.13 机器离线装 PPT 依赖会失败**（告警降级，模型回退系统 python）；如需兼容改回 `-Versions 312,313,314` 并把 `_SUPPORTED_MIN` 调回 `(3,12)`。
- **验证**：prune 脚本 ruff check/format、mypy --strict 零错误，dry-run + 实跑均正确（精确保留 6 驱动）；bootstrap 15 用例全绿（越界用例锁 3.13、上界 3.14 在范围内）；fetch 脚本 PowerShell 语法通过；种子 schema 校验 OK。

### v2.128 — 本地知识库混合检索 RAG 落地：审核专家上传参考资料 + 聊天 KB 检索共用（FTS5 BM25 + 向量 + RRF + reranker）

- **背景**：`knowledge/` 模块此前全是占位桩——`RAGRetriever.retrieve()` 返 backend="stub" 空结果、`KnowledgeIngestion`/`chunker` 全 not implemented；聊天图虽已连 `intent → rag_retrieve → decompose`（`rag_enabled` 默认 True）但检索恒空。审核专家（文档审核）也仅有 grep 式知识库引用与财税规则混合检索，缺用户上传参考资料的 RAG 通道。本期把 `knowledge/` 填实为审核专家 + 聊天共用的本地混合检索引擎，全程离线、单文件复制即迁移。
- **改动**：① **混合检索引擎** [hybrid_rag.py](../services/agent/src/agent/knowledge/hybrid_rag.py)——稀疏通道用 **SQLite FTS5 原生 bm25()**（[tokenizer.py](../services/agent/src/agent/knowledge/tokenizer.py) jieba 分词，索引/查询期都切词后喂 unicode61，解决中文 BM25 与 GB/T 22239 类条款编号匹配；jieba 缺失退化 CJK bigram）+ 稠密通道 sqlite-vec 余弦（复用 [vector_store.py](../services/agent/src/agent/vector_store.py)），**RRF 倒数排名融合**（免调参），再经 **ONNX cross-encoder reranker**（[reranker.py](../services/agent/src/agent/knowledge/reranker.py)，bge-reranker 量化版随包分发，模型缺失自动 no-op 保持 RRF 序）重排取 Top-K；元数据过滤（category/source_type/doc_id）、任一通道故障 best-effort 降级（向量挂→纯 BM25、FTS 挂→纯向量、都空→LIKE 兜底、库空→短路不触发 embedding）。② **父子两层 + 上下文前缀** [chunker.py](../services/agent/src/agent/knowledge/chunker.py)——标题感知（markdown #/中文条款「第X章/第X条」）切分，父块 ~2000 字承载上下文、子块 ~512 字聚焦检索（small-to-big：命中子块回喂父块给 LLM），层级标题路径拼到子块索引文本开头（无 LLM 的 Contextual Retrieval），保留页码元数据（按 parse_document 块偏移映射）。③ **上传入库** [ingestion.py](../services/agent/src/agent/knowledge/ingestion.py)——校验扩展名/大小 → 复制源文件入数据根 → 复用 [doc_review.parser](../services/agent/src/agent/doc_review/parser.py) 解析 → 分块 → 进程内 ONNX bge 批量向量化 → [storage.py](../services/agent/src/agent/knowledge/storage.py) 落库（docs/parents/chunks + FTS5 + vec0 + kb_meta 同库），状态机 pending→indexing→ready/failed；`reindex()` 基于库内原文重嵌入（模型/维度漂移自愈，不依赖源文件原路径）。④ **数据根迁移自包含** [rag_config.py](../services/agent/src/agent/knowledge/rag_config.py)——kb.db + 上传文件 files/ + 参数 JSON 全落 `data_root()/knowledge/`（生产=安装目录），库内零绝对路径，复制该目录到新环境即用；参数保存**不热应用**（写 JSON，重启 Agent 后 `load_rag_config` 生效）。⑤ **两端接入**：审核专家 [doc_review/api.py](../services/agent/src/agent/doc_review/api.py) `_run_analysis` 在财税规则检索后追加 KB 混合检索，召回片段作为独立 `{{rag_context}}` 槽位注入四个 analyze_*.yaml（带编号引用 + 禁止编造指令，关闭/库空时行为与现状完全一致）；聊天 [rag_retrieve.py](../services/agent/src/agent/graph/nodes/rag_retrieve.py) 触发词扩充（合规/审核/制度/法规/风险…）+ trace 透出 refs（source/snippet/page/matched 供前端引用卡），填实 retriever 后自动生效。⑥ **端点/前端/Tauri**：`/knowledge/v1/*`（upload/list/delete/search/reindex/status/config）；审核页新增 [KbPanel.tsx](../apps/desktop/src/components/doc-review/KbPanel.tsx)（上传/列表/状态徽标/删除/索引轮询），设置页新增 [KnowledgeRagPanel.tsx](../apps/desktop/src/views/settings/KnowledgeRagPanel.tsx)（`/settings/knowledge`，参数编辑 + 重启提示 + 一键重启 + 概览 + 重建索引）；Tauri 新增 [commands/knowledge.rs](../apps/desktop/src-tauri/src/commands/knowledge.rs) `_op` 分发。⑦ **LLM 增强阶段**（HyDE / Query Expansion / LLM 上下文前缀）留可插拔 seam + 配置开关，默认关；启用时经 `build_default_rag_llm` 走 `LMRouter.generate_review`——**用「模型管理」里已启用的模型**（同 doc_review 审核姿态，无需为 RAG 单独配置模型），新增 `kb_hyde`/`kb_expand`/`kb_contextual` 三个 TaskKind（不入 `_LOCAL_ONLY_TASKS`，按已启用后端链路由；mock 模式返空串使 seam 优雅 no-op）。⑧ 依赖/打包：新增 `jieba`（纯 Python），[eaide-agent.spec](../eaide-agent.spec) 加 jieba `collect_data_files` + hiddenimport 与 bge-reranker 模型目录（未下载自动跳过），[download_bge_reranker_onnx.ps1](../model/download_bge_reranker_onnx.ps1) 拉取脚本（模型 ~266MB **不入 git**——.gitignore 排除，同 officecli/ppt-master 模式；由 [build-all.bat](../build-all.bat) 与 [release.yml](../.github/workflows/release.yml) 在 PyInstaller 前 best-effort 拉取，再经 spec datas 打进 agent.exe；未拉取时 reranker 静默 no-op）；**未引入 rank-bm25/Qdrant/Chroma/sentence-transformers**（BM25 走 FTS5、向量沿用 sqlite-vec、embedding 沿用进程内 ONNX，契合内网离线 + 安装包体积红线）。
- **验证**：后端新增 [test_knowledge_hybrid_rag.py](../services/agent/tests/test_knowledge_hybrid_rag.py) 28 用例（tokenizer/chunker 父子+页码/storage FTS5+向量+级联删除+漂移/hybrid RRF+降级+rerank+过滤/retriever 编号引用/ingestion 复制入库+reindex/rag_config 重启语义/api 端点）全绿，回归 test_vector_store + test_doc_review_analysis + test_doc_review_knowledge/storage 无失败；前端新增 [kbPanel.test.tsx](../apps/desktop/tests/kbPanel.test.tsx) + [knowledgeRagPanel.test.tsx](../apps/desktop/tests/knowledgeRagPanel.test.tsx) 6 用例、settingsRoutes 补 `knowledge` 段全绿；ruff check/format、mypy --strict（knowledge 仅剩 adapter.py 历史负债）、tsc、eslint 零告警，cargo check 通过。conftest `_isolate` 关 `rag_preload_on_startup` + 重置 knowledge 单例。未新增 SSE 事件（refs 走既有 trace 通道），`_LOCAL_ONLY_TASKS` 不变。**reranker 模型已拉取并端到端验证**：`Xenova/bge-reranker-base` 量化版（tokenizer.json 16.3MB + onnx/model_quantized.onnx 266.4MB，`model/download_bge_reranker_onnx.ps1`），session 输入仅 input_ids+attention_mask（XLM-R 无 token_type_ids，feed 按 `get_inputs()` 动态构造），输出 [N,1] logit，实测相关文档得分最高、无关文档强负。LLM seam 新增 4 用例（HyDE/扩展/上下文前缀启用时用已启用模型 + 关闭时 no-op），test_knowledge_hybrid_rag 共 32 用例全绿；router/llm/doc_review 回归无失败。

### v2.127 — 裁剪 ppt-master 非运行时对比样图（palette/type gallery，−~19MB）

- **背景**：承接安装包瘦身排查，`vendor/ppt-master/references/ai-image-comparison`（44MB 对比样图库）疑似非运行时必需。核查确认：Confirm UI（`scripts/confirm_ui/server.py`）的 `/ai-image-comparison/<kind>/<filename>` 路由对 `kind != 'rendering'` 直接返 404，`_build_ai_image_comparison()` 仅返回 `rendering`；README 明示 `palette/`=legacy 诊断材料（"not a runtime catalog"）、`type/`=内部构图参考；`prompt_audit.py`+manifest 为 maintainer-only（"never loaded as prompt context"）；`attribution_guard.py` 完整性闸门只校验 LICENSE/SPONSORS + 特定脚本执行闸门，不碰 references。
- **改动**：裁掉 `palette/`（11.7MB）+ `type/`（7.6MB）≈19MB（PNG 不可压缩，实打实进安装包），保留 `rendering/`（23.9MB，Confirm UI 运行时 serve）+ README。落地为构建期幂等裁剪：[fetch-ppt-master.ps1](../infra/scripts/fetch-ppt-master.ps1) 新增 step 1b（每次调用都 prune，含已 fetch 的本地副本）、[build-all.bat](../build-all.bat) 打包前 `rmdir` 兜底（覆盖未重新 fetch 的构建路径）。
- **验证**：fetch 脚本 PowerShell 语法解析通过；裁剪仅删只读样图目录，不影响 attribution_guard 闸门、Confirm UI rendering 选择、image_gen 正常出图（palette/type 仅供维护期再生成/诊断）。
- **备注**：整目录（44MB）不可删——rendering/ 是运行时 Confirm UI 渲染风格选择的图源。

### v2.126 — 安装包去重：config/driver + vendor/officecli 不再打进 agent.exe（onefile 负载 −203MB）

- **背景**：407MB 安装包分析发现 `config/driver`（内网 DB 驱动 wheel ~139MB）与 `vendor/officecli`（OfficeCLI 二进制 ~64MB）被打包了两遍——既在 `eaide-agent.exe`（PyInstaller spec datas）内，又作为 Tauri `bundle.resources` 单独落到安装目录。运行时只会用到安装目录那份，exe 内副本是纯死重量（且 onefile 每次启动还要解压）。
- **改动**：① 核对运行时解析路径确认 exe 内副本从不被读——[driver_bootstrap.py](../services/agent/src/agent/driver_bootstrap.py) `_find_driver_dir` 走 `$EAIDE_DRIVER_DIR` → **exe 所在目录**/config/driver → 项目根，从不含 `_MEIPASS`；officecli 原三级回退含 `_MEIPASS`，故给 [officecli_runtime.py](../services/agent/src/agent/builtin/officecli_runtime.py) `_bundled_candidates` 补上「exe 所在目录」候选（cwd → **exe 目录** → _MEIPASS → 仓库根，镜像 driver_bootstrap），生产模式 Tauri 把 officecli 落到安装目录（= exe 同级）即命中，不再依赖 exe 内副本。② [eaide-agent.spec](../eaide-agent.spec) datas 移除 `('config/driver', ...)` 与 `('vendor/officecli', ...)` 两条（保留 `vendor/executor` ~0.5MB，体积小且 jsonrpc_stdio 仍用其 _MEIPASS 兜底）。③ CI/release 工作流清理已失效的 `vendor/python` 占位（承接 v2.124）。
- **影响**：agent.exe 去掉 ~203MB 近乎不可压缩的重复负载（wheel/二进制 UPX 压不动），onefile 启动解压也随之变快；安装目录的 config/driver、vendor/officecli 由 Tauri resources 照常提供，功能不变。**注意**：agent.exe 由此不再自包含——必须与 Tauri resources（安装目录的 config/driver、vendor/officecli）同级分发（生产本就如此），单独裸跑 exe 不再自带驱动/OfficeCLI。
- **验证**：新增 [test_builtin_office.py](../services/agent/tests/test_builtin_office.py) `test_bundled_candidates_include_exe_dir_when_frozen`（frozen 时候选含 exe 所在目录）；office/preview 用例（含既有 `_MEIPASS` 候选用例）全绿；spec `ast.parse` 通过；ruff check/format、mypy --strict 零错误。

### v2.125 — 回答逐字流式输出（answer_delta）：终答从「整段下发」升级为逐 token 流式

- **背景**：传输层 SSE 早已打通（后端 StreamingResponse → Rust sse_bridge → 前端 useAgentStream），但所有 LLM 调用均 `stream: False`，终答要等 responder 节点跑完才经 `message` 事件整段下发——长回答时用户盯着空屏幕等十几秒。本期把 responder 终答路径（summarise 家族：MAIN_AGENT 直答 / 中段闲聊 / 工具结果综合 / 中文重写 / 子 Agent 汇总）改为逐 token 流式；工具循环 FINAL_ANSWER（JSON 动作协议）不在本期范围。
- **改动**：① **流式原语**：新增 [stream_utils.py](../services/agent/src/agent/llm/stream_utils.py) `ThinkBlockFilter` 增量过滤器（跨 delta 边界抑制 `<think>`/```think 块，扣留尾巴 flush 放行，防思考流漏给前端）；[ollama.py](../services/agent/src/agent/llm/ollama.py) 新增 `chat_stream`（NDJSON 逐帧，复用熔断/keep_alive/num_ctx/num_predict，done 帧记 Token 计量）；[private_llm.py](../services/agent/src/agent/llm/private_llm.py) 新增 `chat_stream`（OpenAI SSE `data:` 行解析，4xx 先 aread 再拼错误正文同 #137 语义）。② **事件通道**：[builtin/events.py](../services/agent/src/agent/builtin/events.py) 新增 `EVT_ANSWER_DELTA` + `emit_answer_delta`（复用 shell_chunk 同款进程内 deque，stream.py 0.4s 轮询 drain，零新增管道）。③ **路由层**：[router.py](../services/agent/src/agent/llm/router.py) 新增 `build_summarise_messages`（与各后端 summarise 同源上下文，末尾追加覆盖 Strict JSON 的纯文本输出指令）+ `LMRouter.summarise_stream`（mock/L1 命中单帧直出；降级链取首个具 chat_stream 后端；首 delta 前失败切下级、已发 delta 后失败直接回退非流式防双段草稿；L1 用独立 task_kind 命名空间防 sources 交叉污染）。④ **responder 接入**：[responder.py](../services/agent/src/agent/graph/nodes/responder.py) `_summarise_maybe_stream` 替换全部 5 处 `llm.summarise` 调用点；新配置 `answer_stream_enabled`（默认开，关闭即回退整段下发）。⑤ **msgId 协同**：[stream.py](../services/agent/src/agent/graph/stream.py) 从首条 answer_delta 种子化 `final_answer_msg_id`，终答 `message` 事件复用同一 id（#142 原地覆盖），草稿与终稿收敛为同一条气泡；builtin drain 前置到图块转换之前，同批到达也不会发出两个 id。⑥ **SSE `answer_delta` 三处同步**（CLAUDE.md §4）：stream.py `_CHANNEL_BY_KIND` + [sse_bridge.rs](../apps/desktop/src-tauri/src/stream/sse_bridge.rs) channel/映射 + [events.ts](../apps/desktop/src/ipc/events.ts) EVT；线协议 [shared-protocol](../packages/shared-protocol) TS/Python 双侧镜像同步（AnswerDeltaEvent）。⑦ **前端**：[useAgentStream.ts](../apps/desktop/src/hooks/useAgentStream.ts) 新增 `case 'answer_delta'`（首帧 append 草稿气泡、后续帧按 msgId 原地拼接、runId→页签路由不串台），流式光标复用现有 busy 渲染。
- **验证**：后端新增 [test_answer_stream.py](../services/agent/tests/test_answer_stream.py) 19 用例（过滤器跨边界/未闭合/flush、ollama NDJSON、private SSE、流式链降级三态、responder 发 delta + 开关回退、stream.py msgId 种子化）全绿，全量 pytest 无失败；前端新增 [answerDeltaStream.test.tsx](../apps/desktop/tests/answerDeltaStream.test.tsx) 4 用例，全量 355/355；ruff check/format、mypy packages、tsc、eslint 零告警；cargo check 通过。未新增 `_LOCAL_ONLY_TASKS` 任务类型（流式仅改输出形态，敏感任务本地红线不变）。

### v2.124 — PPT Master 去内嵌 Python：解释器复用设置→工具链 + deps 多版本离线 wheel（安装包预计 −181MB）

- **背景**：V9.7 随包捆绑嵌入式 CPython 3.12（`vendor/python`，含预解压 `ppt-master-site` 共 ~181MB），是 407MB 安装包的最大单一可裁剪项。用户要求不再打包 Python，直接复用「设置 → 工具链」里配置的 Python 路径。
- **改动**：① [ppt_master_bootstrap.py](../services/agent/src/agent/ppt_master_bootstrap.py) 重写——`resolve_ppt_master_python()` 复用 [toolchain.resolve_toolchain](../services/agent/src/agent/coding/toolchain.py)（设置 configured → PATH → 常见目录三级回退，不进程内缓存以便改配置即生效）；`ensure_ppt_master_runtime()` 改为用解析到的解释器 `pip install --no-index --find-links deps -r requirements-offline.txt` 离线装依赖（marker 落 data_root 幂等；`_probe_version` 越界 3.12–3.14 告警降级）；新增 `substitute_python_token()` 把种子 prompt 的 `{{PPT_MASTER_PYTHON}}` 占位替换成解析路径；`EAIDE_SKIP_PPT_BOOTSTRAP` 环境变量供测试隔离 / 精简部署 opt-out。② 种子 [office_pptx_designer.yaml](../services/agent/src/agent/skills/seeds/office_pptx_designer.yaml)（v5.1.0）【Python 解释器】改为运行时解析注入（[loop.py](../services/agent/src/agent/tools/loop.py) `_active_skill_addon` + [router.py](../services/agent/src/agent/skills/router.py) `build_system_prompt` 两处替换 token；无占位符的 skill 原样返回不探测）。③ **deps 支持 3.12–3.14**：[fetch-ppt-master.ps1](../infra/scripts/fetch-ppt-master.ps1) 移除内嵌 Python 下载步骤，改为按 312/313/314 三版本循环（版本列表已参数化 `-Versions`，便于日后增减） `pip download`（abi3 / 纯 py wheel 天然共用去重），并生成 UTF-8 无 BOM 的 `requirements-offline.txt`（剔除仅联网可得的 curl_cffi / google-genai，防 `--no-index` 整体失败）；安装时 pip 按当前解释器自动挑兼容 wheel，bootstrap 无需版本判断逻辑。④ **打包/构建**：[tauri.conf.json](../apps/desktop/src-tauri/tauri.conf.json) 移除 `vendor/python/` 资源；[eaide-agent.spec](../eaide-agent.spec)、build-all.bat、.gitignore 注释同步。
- **验证**：重写 [test_ppt_master_bootstrap.py](../services/agent/tests/test_ppt_master_bootstrap.py) 15 用例（解释器解析 / token 替换 / 离线装 `--no-index --find-links` / marker 幂等 / 新 wheel 触发重装 / pip 失败记 marker / 版本越界(3.11)告警 + 3.14 上界内不告警 / 前置缺失友好降级；subprocess 与 toolchain 全 mock，不联网不污染 venv）；conftest `_isolate` 置 `EAIDE_SKIP_PPT_BOOTSTRAP` 防 `import agent.main` 触发真实 pip；[installerIncremental.test.ts](../apps/desktop/tests/installerIncremental.test.ts) 断言 resources 不再含 `vendor/python`（4/4）；技能 / 播种 / 工具链回归全绿，ruff check + format、mypy --strict 零错误。deps 由 cp312 单版本（~52MB）扩到 3.12–3.14（重跑 fetch 后 ~100MB），去 181MB 内嵌 Python；因内嵌 Python 的预解压 site 在 NSIS 下可压缩、而多版本 wheel 不可压缩，此项对安装包的净收益以重建实测为准（版本范围最终定为 3.12–3.14，兼顾体积与本机 3.14）。

## 2026-09-01

### v2.123 — 向量存储统一迁至 sqlite-vec：全仓向量与本地向量模型端侧闭环

- **背景**：此前向量散落四处且形态不一（intent_memory 的 vec_json 文本列、语义路由的 JSON 缓存文件、L2/财税/NL2SQL 的进程内存缓存、knowledge 预留未落地），重启即失效且无统一检索入口。统一迁到 SQLite sqlite-vec 扩展（vec0 虚拟表），与进程内 ONNX 向量模型（bge-small-zh-v1.5）形成纯本地闭环。
- **改动**：① 新增统一访问层 [vector_store.py](../services/agent/src/agent/vector_store.py)：float32 BLOB 序列化 / vec0 建表（维度漂移自动重建）/ upsert（零向量拒绝入库；vec0 不支持 INSERT OR REPLACE，先删后插）/ `vec_distance_cosine` 余弦扫描（与原内存 _cosine 完全同语义，零向量回落 0.0）；aiosqlite 经 worker 线程调度（`run_async`）。② **intent_memory**：案例向量迁 `intent_examples_vec`，检索改 SQL JOIN 评分（正样本 +0.02 加权不变）；存量 `vec_json` 首连自动迁入并 DROP 旧列。③ **语义路由**：示例句/困难负样本向量缓存由 `semantic_route_cache.json` 迁 `semantic_route.db`（route_vec + 指纹），命中/拦截/阈值语义不变。④ **L2 语义缓存**：兑现 docstring 承诺，进程内 dict 快路径 + router.db 持久层（l2_cache + l2_cache_vec），重启后仍可语义命中；敏感任务不缓存红线不变。⑤ **财税规则库 / NL2SQL linker**：语料/表向量新增持久缓存（`doc_review_vec.db` / `schema_link_vec.db`，按签名/指纹失效），重启免全量重新向量化；几百张表的 schema 链接不再每问重算。⑥ **知识库**：`KnowledgeStorage` 分块向量层落地（upsert_chunks / search_by_vector / search_by_text / delete_chunks_by_doc，kb_chunks_vec），文档级 CRUD 仍占位。⑦ 依赖：`sqlite-vec>=0.1.9`（agent 包，带 win_amd64 轮子，无需编译）；扩展加载失败全部静默回退（返空/现算），不阻塞主链路。
- **验证**：新增 [test_vector_store.py](../services/agent/tests/test_vector_store.py) 9 用例（基础层 / L2 跨实例命中 / KB 分块检索 / 存量迁移），语义路由缓存用例改断言 sqlite 库；全量回归 2829 用例 0 失败（9 个环境性跳过）；ruff check / format 全绿。未新增 SSE 事件，`_LOCAL_ONLY_TASKS` 不变。
