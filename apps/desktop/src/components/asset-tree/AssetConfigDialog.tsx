...
const DB_TYPE_OPTIONS: DbTypeInfo[] = [
  // 主流
  {
    ...
    defaultPort: 3306,
    category: "mainstream",
    placeholderDb: "my_database",
    placeholderUser: "readonly",
  },
  {
    value: "postgresql",
    label: "PostgreSQL",
    defaultPort: 5432,
    category: "mainstream",
    placeholderDb: "postgres",
    placeholderUser: "postgres",
  },
  {
    value: "oracle",
    label: "Oracle",
  ...
  },
  {
    ...
    defaultPort: 8123,
    category: "mainstream",
    placeholderDb: "default",
    placeholderUser: "default",
  },
  {
    value: "sqlite",
    label: "SQLite",
  ...
  },
  {
    ...
    defaultPort: 5258,
    category: "xinchuang",
    placeholderDb: "gbase_db",
    placeholderUser: "gbase",
  },
  {
    value: "oceanbase",
    label: "OceanBase",
...
export function AssetConfigDialog({
  node,
  ...
  const [type, setType] = useState<AssetNode["type"]>(
  ...
  const handleSave = useCallback(async () => {
    ...
    // database 类型：把 UI 默认展示的 db_type / port 回写进 meta。
    // 否则 Rust 会注入 type="" 导致后端无法分发执行（BUGFIX #97：曾静默返回 0 行）
    let finalMeta = meta;
    if (type === "database") {
      ...
      finalMeta = {
        ...meta,
        db_type: dbType,
        ...(meta.port || info.defaultPort <= 0
          ? {}
          : { port: String(info.defaultPort) }),
      };
    }
    setSaving(true);
    try {
      if (node) {
        await updateAsset(node.id, { label: label.trim(), meta: finalMeta });
      } else {
        await addAsset({
          type,
          label: label.trim(),
  ...
  return (
    ...
    >
      ...
      >
        ...
        {type === "rest" && <RestFields meta={meta} setField={setField} />}
        {type === "ssh" && <SshFields meta={meta} setField={setField} />}
        {type === "rpa" && <RpaFields meta={meta} setField={setField} />}
      ...
      <div className="space-y-2">
        ...
        <div>
          ...
          <Input
            ...
            placeholder={
              dbType === "sqlite" ? "C:/data/my.db" : "C:/data/report.xlsx"
            }
          />
        </div>
    ...
    <div className="space-y-2">
      ...
      <div>
        ...
        <Input
          ...
          placeholder={info.placeholderDb ?? "my_database"}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <Input
            ...
            placeholder={info.placeholderUser ?? "readonly"}
          />
        </div>
        <div>
          ...
          <Input
            ...
            placeholder="__KEYRING_REF:db_xxx"
          />
        </div>
      </div>
    ...
    <div className="space-y-2">
      ...
      <div>
        ...
        <Input
          value={meta.password ?? ""}
          onChange={(v) => setField("password", v)}
          placeholder="__KEYRING_REF:ssh_web1"
        />
      </div>
      <div>
        <Label text="Private Key Ref (可选)" />
        <Input
          value={meta.private_key_ref ?? ""}
          onChange={(v) => setField("private_key_ref", v)}
          placeholder="__KEYRING_REF:ssh_key_web1"
        />
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <Input
            value={meta.password ?? ""}
            onChange={(v) => setField("password", v)}
            placeholder="__KEYRING_REF:rpa_hr"
          />
        </div>
      </div>
      <div>
